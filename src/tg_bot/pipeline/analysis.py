...
# USD per 1M tokens for (provider, model) pairs that we render an estimated
# cost for in `/status`. Best-effort — pricing changes constantly upstream
# and this dict is a manual point-in-time snapshot, not an authoritative
# source. The bot also surfaces raw input/output token totals so the operator
# can always do the math themselves against whatever rates they're paying.
#
# Pairs not in this dict fall through to "tokens only" rendering. Adding a
# new provider/model is cheap; not adding one is also fine.
#
# Keys are lower-cased on lookup, so spelling case in `.env` doesn't matter.
# Pricing as of late 2025 (USD per 1M tokens, input → output):
