...
- **Three follow-ups to the email-mirror PR: user-tz digest date, token cost in `/status`, and `/email diagnose`.** (1) `run_user_digest` now derives `today_date` from `digest.tz` via `ZoneInfo`, threading the user-local date through the cache key, market-calendar gate, and rendered header — fixes the Tokyo-at-09:00-JST = UTC-prior-day failure mode noted as a Known Limitation since PR #71. `cache.today_iso(tz)` accepts the tz arg; manual `/watch` taps keep the no-arg server-local form. (2) `_DelegatingProgressCallback.on_llm_end` accumulates `(input, output)` tokens into module-level counters under `_token_counter_lock`; `/status` renders `• Tokens since boot: 1.2M in / 567K out (~$12.34)` via `estimate_token_cost_usd` against `LLM_PRICE_USD_PER_M` (falls back to tokens-only for models not in the price table). (3) New `/email diagnose` sub-command pings `Resend.Domains.list()` + sends a test email + reports all four gates (env vars, domain verification, send) in one MarkdownV2 message — one-shot operator check for "did I configure Resend correctly?". Pinning: 1 new `test_digest.py` scenario for the tz fix (pinned moment 21:00 UTC = 06:00 JST next day), 6 new `test_progress.py` scenarios for token extraction across OpenAI/Anthropic naming + thread-safe accumulation, 3 new `test_email_client.py` scenarios for `/email diagnose` happy/pending/API-error paths.
