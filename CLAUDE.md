...
- **Digest summary now carries a user-visible email-status footer.** Before, the email mirror was completely opaque to the user — they ran `/email <addr>`, the digest fired, and they had to check their inbox (or the bot logs) to know whether the email landed. Now `run_user_digest` issues a SECOND `edit_message_text` after the primary summary edit, appending one of: `📧 Sent to <addr>` (success), `📧 Email failed — check logs` (Resend exception or malformed response), or `📧 Email opt-in but server not configured` (opt-in but `RESEND_API_KEY`/`RESEND_FROM` missing in `.env`). Non-opted-in users see no footer — Telegram-only UX is preserved exactly. Two-edit shape is load-bearing: primary summary is committed BEFORE the Resend call, so the email-mirror invariant (Telegram never blocked by email) extends to the rendering layer; a failed second edit logs INFO and leaves the primary summary + email send intact. `send_digest_email` was refactored to return `EmailSendResult(ok, recipient, message_id, error)` instead of `bool` — the dataclass carries the recipient (for the `Sent to <addr>` rendering) and the exception class name (for log correlation when chasing a failure footer). Pinning: 4 new `test_digest.py` scenarios (success, failure, env-missing, no-opt-in) + 4 updated `test_email_client.py` scenarios (return-shape coverage for each branch).
