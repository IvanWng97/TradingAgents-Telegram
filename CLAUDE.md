...
- **Digest summary now carries a user-visible email-status footer.** Opted-in users see `📧 Sent to <addr>` / `📧 Email failed — check logs` / `📧 Email opt-in but server not configured` appended via a second `edit_message_text` after the primary summary. The two-edit shape preserves the email-mirror invariant (Telegram never blocked by email) at the rendering layer. `send_digest_email` returns `EmailSendResult(ok, recipient, message_id, error)` instead of `bool` so the renderer can format the footer without re-reading storage. Pinning: 4 new `test_digest.py` scenarios + 4 updated `test_email_client.py` scenarios.
