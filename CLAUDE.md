...
- **Per-user config** lives in `data/user_config.json`, keyed by stringified Telegram user_id, holding `llm_provider` + `deep_think_llm` + `quick_think_llm` and an optional `digest: {enabled, hour_local, tz, chat_id, tickers: list[str]}` block. `tickers` is the digest filter — explicit opt-in, default `[]` for new users, fan-out skips with a reminder when empty. `set_llm_provider` wipes deep/quick (provider-specific). `clear(user_id)` is selective — it only pops LLM keys, so a `/config` Cancel rollback doesn't wipe an unrelated digest schedule. The user entry is dropped only when nothing else (e.g. no digest) is left in it.
