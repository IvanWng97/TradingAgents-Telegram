...
| `/digest` | Single-screen picker for a daily watchlist run. First-time users land on the tz picker (10 IANA zones); returning users see a 6×4 hour grid with ✅ on the active hour. `🌍 Time zone` swaps to the tz picker, `▶ Run now` triggers an immediate fan-out, `🔕 Off` disables (preserves hour + tz for one-tap re-enable). Hour selection captures `chat_id` from the live update so the JobQueue callback can send unsolicited messages later. |
