...
- **Same-day result cache.** `cache.py` is a filesystem-backed cache keyed on `(provider, deep, quick, ticker, date_iso)` — no `user_id`, so two users on the same `/config` share results within a day, and a manual `/watch` followed by the digest's auto-fire on the same ticker only pays for one LLM run. Layout: `<TG_BOT_DATA_DIR>/result_cache/<config_slug>/<TICKER>/<date>.json`. Atomic writes (tempfile + fsync + rename, same pattern as `JsonStorage._save`). Hits in `_run_analysis_for_ticker` short-circuit before the semaphore acquire — no progress flow, no Telegraph re-publish; the cached `telegraph_url` is reused. `/refresh <TICKER>` invalidates today's entry then runs through the miss path. Lazy eviction: a fresh date for the same `(config, ticker)` sweeps stale-date siblings on store.
