...
- **Daily digest scheduler.** `/digest` opens a single-screen inline-button picker — 6×4 hour grid + `🌍 Time zone` swap-grid for 10 IANA zones, `▶ Run now` for an immediate fan-out, `🔕 Off` to disable (preserves hour + tz). `digest:` callback prefix carries 6 sub-actions (`hour:HH`, `tz:<IANA>`, `tzpick`, `hourpick`, `off`, `run`). Persisted in `user_config.json` under a `digest: {enabled, hour_local, tz, chat_id}` block; PTB `JobQueue.run_daily` is reconstructed from storage at `_post_init`. Fan-out reuses `_run_semaphore` so it interleaves with manual `/watch`. `Forbidden` on send → auto-disable + JobQueue cancel. `tzdata` PyPI dep added because `python:3.14-slim` doesn't ship the IANA db. `/status` surfaces "Next digest: 10:00 PT (in 4h 23m)" when active. Smoke coverage: 28 scenarios in `scripts/smoke_digest.py` covering storage CRUD, picker rendering, callback dispatch, JobQueue register/replace/cancel, and full fan-out with mocked analysis.
