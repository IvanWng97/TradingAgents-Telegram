...
- **tradingagents pinned to `v0.3.0` + drift sync.** The dep was a bare-HEAD git pin, so local/CI resolved 0.2.5 while Docker rebuilds pulled post-0.3.0 HEAD; now pinned to `@v0.3.0` (pyproject.toml) for reproducibility. Drift fixed: (1) deleted the now-dead `_apply_local_effort_overlay` + `_LOCAL_EFFORT_OVERRIDES` — upstream's `_ENV_OVERRIDES` natively maps `TRADINGAGENTS_OPENAI_REASONING_EFFORT` / `_ANTHROPIC_EFFORT` / `_GOOGLE_THINKING_LEVEL` onto the same config keys as of 0.3.0; `EFFORT_KEY_BY_PROVIDER` stays as the read-side map for `from_config`. (2) `PROVIDER_ENV_KEYS` gained the six 0.3.0 providers (`mistral`/`kimi`→MOONSHOT/`groq`/`nvidia`/`openai_compatible`; `bedrock`→None) so the missing-key precheck stops silently passing then 401-ing. (3) `start.sh` onboarding model IDs refreshed off the 0.3.0 catalog (retired `grok-4.20-*`→`grok-4.3`, `gemini-3-flash-preview`→`gemini-3.5-flash`, `qwen3.6-flash`→`qwen3.7-plus`; trailing flagships bumped). (4) the import guard now catches `ValueError` (0.3.0 fails loud on an invalid bool/int `TRADINGAGENTS_*` value at import) so a malformed `.env` degrades to `TRADINGAGENTS_AVAILABLE=False` instead of crashing startup. (5) adopted the new cross-provider `TRADINGAGENTS_TEMPERATURE` knob into `AnalysisConfigKey` (slug/caption/title) — omitted when unset (upstream default None) so existing cache slots survive. Pinning: overlay tests in `test_pipeline_analysis.py` replaced by `from_config` effort-resolution scenarios; `test_progress.py` upstream-node pin bumped `_V025`→`_V030` (the social-analyst node is now `"Sentiment Analyst"`, already covered by the `_STEP_MAP` alias). Verified safe with no change: on-disk log path/`full_states_log_<date>.json` shape, `propagate()` return tuple, and `final_state` key set are unchanged 0.2.5→0.3.0, so `/history`, the 7-section packer, and the cache need no edits. Declined #92 (langchain-toolstem): the bot can't wire a data vendor without forking tradingagents (closed `VENDOR_METHODS`), and 0.3.0's verified-data contract + the free `alpha_vantage` vendor already address the silent-yfinance premise.
