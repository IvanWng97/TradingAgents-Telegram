...
5. **If any `CLAUDE.md` (root or nested) is touched**, the change must go through the `claude-md-management` skill for review/approval before merge. The skill does Discovery → Quality Assessment → Quality Report → Targeted Updates and catches drift between sibling files, scope creep, and bullets that grew past their useful density. Don't hand-edit a CLAUDE.md and skip the audit pass.
