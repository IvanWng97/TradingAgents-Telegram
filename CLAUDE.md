...
- **Digest date semantics are server-UTC, not user-local**: `run_user_digest` uses `date.today()` (server's local date — typically UTC inside Docker) for both the calendar gate (`market_calendar.is_market_open_for(ticker, today_date)`) AND the cache-key date AND the rendered header date. For a user with `digest.tz = "Asia/Tokyo"` firing at 09:00 JST = 00:00 UTC the prior day, all three reference the prior date. The cache key and rendered date have been this way since the cache was added; PR #71 inherits the same convention for the gate to avoid introducing a per-component date asymmetry (gate using local while cache uses UTC would split entries between two date slots for the same user). Fix is a single follow-up refactor of `today_iso` semantics to accept a tz parameter — out of scope for the gate-only PR. Symptom for impacted users: a Tokyo-only watchlist firing at 09:00 JST sees the gate evaluate "yesterday's UTC date" against the calendar — for Tokyo's regular Mon-Fri sessions this happens to give the right answer most of the time (UTC-prior is also a weekday), but holiday-boundary days can produce wrong skips. The architect review for PR #71 (in git log) traces the full failure mode.
...
- **Progress callback: diagnostic surfaces bumped to INFO/WARNING + `_STEP_MAP` aligned with tradingagents v0.2.5.** A VPS deploy showed users seeing `📊 Analyzing BRK-B — running Sentiment Analyst…` (no `(n/12)` badge) and assuming the analysis had wedged. Two distinct issues compounded: (1) every silent-failure surface in `pipeline/progress.py` logged at DEBUG (invisible at production levels), so a wedge was indistinguishable from "no callbacks fired"; (2) upstream tradingagents issue #557 renamed `create_social_analyst` → `create_sentiment_analyst`, and newer pip-from-git builds emit `"Sentiment Analyst"` as the node name directly — not in our `_STEP_MAP`, so the badge silently disappeared. Now: every silent drop in `_dispatch` (no reporter on thread, missing `langgraph_node` metadata, failed `run_coroutine_threadsafe`) logs INFO/WARNING with diagnostic context; `report()` logs INFO on successful caption edits AND WARNING on Telegram failures (was DEBUG-swallowed); `resolve_step()` logs WARNING listing the expected vocabulary when a node name doesn't resolve (canary for future upstream renames). `_STEP_MAP` adds a `"sentiment analyst"` alias → step 2 (back-compat for either node-name variant) and drops the dead `"risk judge"` alias (no corresponding `add_node()` in v0.2.5). Pinning: new `tests/test_progress.py` with 13 scenarios — 3 for `_STEP_MAP` upstream-alignment (`test_step_map_covers_all_upstream_v025_nodes` pins every v0.2.5 LLM-node name resolves with an ordinal; `test_total_steps_derived_from_step_map_max_ordinal` pins `TOTAL_STEPS = max(...)` is derived not hardcoded; `test_step_map_sentiment_analyst_alias_resolves_to_social` pins the issue #557 alias), 3 for `resolve_step` behavior, 4 for `_dispatch` log surfaces, 3 for `report` behavior. Build-time canary (the alignment test) + runtime canary (the new WARN logs) cover both directions of drift.
