...
- **Digest summary now carries a user-visible email-status footer + `/email` UX parity with `/add`.** Opted-in users see `📧 Sent to <addr>` / `📧 Email failed — check logs` / `📧 Email opt-in but server not configured` appended via a second `edit_message_text` after the primary summary. The two-edit shape preserves the email-mirror invariant (Telegram never blocked by email) at the rendering layer. `send_digest_email` returns `EmailSendResult(ok, recipient, message_id, error)` instead of `bool` so the renderer can format the footer without re-reading storage. Plus a UX pass: bare `/email` now opens a `ForceReply` prompt (matching bare `/add`) — current email moved to `/status`. New `email_via_reply` handler registered in PTB group=1 alongside `add_via_reply` in group=0 (strict prompt-equality match keeps them isolated). Pinning: 4 `test_digest.py` scenarios + 11 `test_email_client.py` scenarios across return shape, `__post_init__` guard, `/email test` regression, and the ForceReply UX.
