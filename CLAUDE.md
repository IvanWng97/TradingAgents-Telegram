...
- **Folder reorg: `pipeline/` + `rendering/` subpackages + `handlers/pickers.py` extraction.** The 14 flat files at `src/tg_bot/` got grouped into two new subpackages (`pipeline/` for the LLM execution hot path — `analysis.py`, `cache.py`, `config_key.py`, `progress.py`; `rendering/` for data → user-visible output — `formatters.py`, `telegraph_client.py`, `chart.py`). 7 files moved via `git mv`; 6 leaf primitives stayed at parent level (each is a single-concept module with no shared cohesion axis; named subpackage for them was rejected as either stutter-y or arbitrary by two independent architectural reviews). Separately, the 5 keyboard/response builders that lived in `handlers/commands.py` but were imported by `handlers/callbacks.py` got extracted to a new `handlers/pickers.py` — fixes a dependency-direction smell (the dispatcher file importing from the entry-point file) and shrinks `commands.py` from 812 → ~600 LOC. The `analysis_runner.py` file (1,328 LOC) is deliberately untouched — its shared-mutable-state coupling per Invariant #5 makes decomposition a separate higher-risk PR. Smoke fixture monkey-patches that reach into `tg_bot.analysis._graph_pool` and similar attributes were updated automatically by the sed pass since they import via `from tg_bot.pipeline import analysis as analysis_mod`. The decision to keep `auth.py`/`validation.py`/`history.py`/`digest.py` flat at parent (rather than push them into `handlers/`) was a deliberate guardrail: keeping `handlers/` narrow to "registers PTB handlers OR is direct callback impl" prevents the junk-drawer drift that "handler-layer code" would invite.
