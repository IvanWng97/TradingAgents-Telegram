...
1. **Verify the code works.** `.venv/bin/python -m ruff check src/ scripts/ && ruff format --check` and EITHER `bash scripts/run_smoke.sh` (legacy bash aggregator — runs each `scripts/smoke_*.py` as its own Python process, sequential) OR `.venv/bin/python -m pytest scripts/` (pytest discovers every `async def test_*` in the smoke files via `[tool.pytest.ini_options]` in `pyproject.toml`; `pytest-asyncio` `asyncio_mode = "auto"` handles the event loop). Both runners cover the same 223 scenarios and stay green in lockstep — pytest gives selective run + assertion introspection + per-test GH Actions reporting; bash gives one-process-per-suite hard isolation (no module state leaks between suites). Shared fixtures (`FakeBot`, `FakeContext`, `set_smoke_data_dir`) live in `scripts/_smoke_helpers.py` — used by `smoke_runner.py` + `smoke_runner_parallel.py` (both drive `analysis_runner`); the unit-flavoured suites stay self-contained. **Every smoke repoints `TG_BOT_DATA_DIR` to a fresh tempdir** so the same-day result cache can't leak between scenarios — but the repoint happens at MODULE-IMPORT time, which is fine under bash (each suite is a fresh process) but means under pytest the env var ends up pointing at whichever smoke imported LAST. Tests that need a guaranteed-fresh tempdir must call `set_smoke_data_dir(...)` inside the test body, not rely on the module-level call (see `smoke_runner_parallel.py:test_real_parallelism` for the canonical pattern). `scripts/conftest.py` carries cross-cutting setup: `sys.path` wiring + session-scoped digest precheck disable (replaces the per-script `_disable_llm_precheck_globally()` call that used to live in each `main()`). For UI-affecting changes, also restart the bot and exercise in Telegram — smoke tests verify code correctness, not feature correctness.
...
- No structured logging / correlation id. Smoke coverage exists (`scripts/smoke_*.py`) and runs under both `bash scripts/run_smoke.sh` and `pytest scripts/` — the legacy bash aggregator is kept for hard process-isolation between suites. Graceful shutdown is wired (`_post_stop` signals every in-flight cancel + 2s drain).
