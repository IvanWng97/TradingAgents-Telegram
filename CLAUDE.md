...
- **Review-hardening pass (H1 + 4 fixes from the whole-codebase audit).** (H1) Auth gate no longer fails OPEN: the "Not authorized" notification is now wrapped in try/except so a failing `query.answer`/`reply_text` (e.g. `BadRequest: Query is too old` on a stale button — deterministic) can't skip the `raise ApplicationHandlerStop` and let the update fall through to handler groups. A global `add_error_handler` (`app._on_error`) backstops it. (M1/M2) Two MarkdownV2 escaping bugs that hard-broke whole messages under normal config: `/status`'s `~$12.34` cost now sits in a code span (bare `.` was reserved); `/email diagnose`'s "skipped" line escapes its `+`. (M4) Digest in-flight cancel no longer logs an ERROR "analysis failed" traceback — `_analyze_one_for_digest` re-raises `CancelledByUserError` before the generic handler, matching the manual path (Invariant #5). (L1) `cache.store`'s lazy sweep is now date-gated — only siblings strictly older than the just-written date are unlinked, fixing out-of-order-midnight and manual-vs-digest (server-tz vs user-tz date) mutual eviction; unparseable stems are never touched. Pinning: H1 fail-open guards + `_on_error` registration, M1/M2 rendered-string pins (`test_commands_render.py`, new), the real `_analyze_one_for_digest` cancel-propagation path (first test to drive it un-stubbed), and date-gated-sweep keep-newer/leave-unparseable scenarios — all verified falsifiable against the un-fixed source.
