...
- **Review batch-2, PR A — `analysis_runner` hardening (M5, L2, L3, L7 + M6/L9 coverage).** (M5) The digest final summary could be overwritten by a late trampolining render owned by a `run_coroutine_threadsafe` `_on_step` still mid throttle-sleep when `gather` returned; a `finished` latch (set before the summary edit, with no `await` between) makes such a render bail on wake. (L2) The four `result_cache.lookup`/`store` calls and `_handle_get_full_md`'s log read + `.md` build now run inside `asyncio.to_thread` — blocking fsync/json off the event loop (was stalling concurrent progress edits + Cancel taps). (L3) Digest cancel now `Task.cancel()`s **only PENDING** tickers; in-flight ones (tracked in `acquired_tickers`, mapped via `task_tickers`, both read by `_cancel_pending_digest_tasks`) unwind via `cancel_event` so the semaphore slot + pool graph release together — cancelling them mid-`propagate()` freed the slot while the thread kept the graph, transiently breaking sem-cap == pool-size. (L7) The two `sem._value`/`_waiters` debug logs read via `getattr` so they can't crash the run on a future CPython attr reshape. Coverage added: M6 (the real `_analyze_one_for_digest` — cache-hit skips the semaphore; in-flight cancel never publishes/stores) and L9 (the `_try_acquire_nonblocking` AttributeError fallback). Every fix verified falsifiable against the un-fixed source (M5's repro shows the late render re-attaching the dead Cancel keyboard over the summary). The 21 digest stub fakes gained `**_kwargs` so the new `acquired_tracker` kwarg (and future ones) don't break them.
