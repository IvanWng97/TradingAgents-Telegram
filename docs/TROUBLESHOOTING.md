...
| Analysis caption stuck at "📊 Analyzing… please wait" with no progress | LLM authorization or rate-limit error. Check `docker compose logs -f` for the actual error from `tradingagents`. Common: provider key for the wrong service (e.g. `OPENAI_API_KEY` set but `.env` has `TRADINGAGENTS_LLM_PROVIDER=deepseek`). |
