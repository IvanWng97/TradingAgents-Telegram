...
| Provider keys | yes (≥1) | `OPENAI_API_KEY`, `DEEPSEEK_API_KEY`, `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `XAI_API_KEY`, `DASHSCOPE_API_KEY` (qwen), `ZHIPUAI_API_KEY` (glm). Fill in as many as you have — the active one is whichever you select via `/config`; the others sit unused. At least one is required so `/config` has something to work with. Ollama needs no key. |
